...
class FrequencyAttention(nn.Module):
    """
    频率注意力机制 (Frequency Attention)
    
    专为EEG频谱图设计的频率维度注意力，让模型能动态感知不同频段的重要性。
    对频率维度应用自注意力，使模型能够学习哪些频率成分更重要。
    
    Args:
        num_channels: 输入通道数
        freq_dim: 频率维度大小 (默认103，对应裁剪后的频率范围)
        reduction_ratio: 降维比率 (默认4)
    """
    ...
    def __init__(
        ...
        # 频率维度平均池化 (沿时间维度)
        self.freq_pool = nn.AdaptiveAvgPool2d((freq_dim, 1))
        
        # MLP 用于学习频率权重
        hidden_dim = max(freq_dim // reduction_ratio, 8)
        self.mlp = nn.Sequential(
            nn.Conv2d(num_channels, hidden_dim, kernel_size=1),
            nn.Conv2d(hidden_dim, num_channels, kernel_size=1)
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 沿时间维度池化，获取频率特征 [B, C, H, 1]
        freq_feature = torch.mean(x, dim=-1, keepdim=True)
        
        # 通道特征变换
        freq_feature = self.mlp(freq_feature)  # [B, C, H, 1]
        
        # 生成频率权重 [B, 1, H, 1]
        freq_weights = self.freq_weight(freq_feature)
...
class NAFBlock(nn.Module):
    ...
    Args:
        ...
        use_freq_attention: 是否使用频率注意力 (默认True)
        freq_dim: 频率维度大小 (默认103)
    """
    ...
    def __init__(
        ...
        if use_freq_attention:
            self.freq_attn = FrequencyAttention(
                num_channels=expanded_channels // 2,
                freq_dim=freq_dim
            )
...
class SpectrogramNAFNet(nn.Module):
    ...
    专为处理 STFT 频谱图设计，包含奇数维度适配器和频率注意力机制。
    ...
    Args:
        ...
        use_freq_attention: 是否使用频率注意力 (默认 True)
        freq_dim: 频率维度大小 (默认 103)
    """
